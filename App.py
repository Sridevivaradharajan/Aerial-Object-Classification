...
GDRIVE_CONFIG = {
    'classification_model': {
        'file_id': '1qZilgEYD2q-Ur6FVnt5ap711oh3fA_in',
...
def load_classification_model():
    """Load TensorFlow classification model (.keras or .h5 format)"""
    try:
        model_path = GDRIVE_CONFIG['classification_model']['output']

        # Download if needed
        if not os.path.exists(model_path):
            with st.spinner("Downloading classification model (1-2 min)..."):
        ...
        # Load model - .keras format is natively supported in TF 2.16+
        # This handles .keras files perfectly and .h5 with compile=False
        ...
        format_type = "Keras 3.0" if model_path.endswith('.keras') else "HDF5"
        return model, f"TensorFlow Model ({format_type})"

    except Exception as e:
        st.error(f"Failed to load classification model: {str(e)}")
        st.info("""
        **💡 Recommended: Save as .keras format**
        ```python
        model.save('model.keras')  # Best for TF 2.16+
        ```
        
        **Alternative: .h5 format**
        ```python
...
GDRIVE_CONFIG = {
    'classification_model': {
        'file_id': '1r_GZRaLXaKmAD8KLlBwgv4hWDQkxmjJR',
        'output': 'Models/best_model_tf.h5' 
    },
    'detection_model': {
        'file_id': '1K-pJncda11qY1JKi9TuOsPtqTiVBoM2Z',
...
def load_classification_model():
    ...
    try:
        ...
        # Load model - Compatible with TensorFlow 2.16+
        model = tf.keras.models.load_model(model_path, compile=False)
        ...
        st.error(f"Failed to load classification model: {str(e)}")
        st.info("Please ensure your model is saved with: `model.save('model.h5', save_format='h5')`")
        return None, None
...
def load_detection_model():
    ...
    try:
        ...
        # Download if not exists
        if not os.path.exists(model_path):
            with st.spinner("Downloading detection model (1-2 min)..."):
                success = download_from_gdrive(
                    GDRIVE_CONFIG['detection_model']['file_id'],
                    model_path
                )
